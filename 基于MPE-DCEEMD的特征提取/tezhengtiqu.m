...
for i=1:size(Ndata,1)
    ...
    imf=MPE_DCEEMD(Ndata(i,:));
    for j=1:size(imf,1)
        pe(i,j)=ipe(imf(j,:));
    end
end

xlswrite('E:\毕业课题\我的进展\课题的改进算法研究\EMD和改进EMD的分类结果对比\基于MPE-DCEEMD的特征提取\正常数据',pe)
%% 对内圈故障数据进行eemd
...
for i=1:size(INdata,1)
    ...
    imf=MPE_DCEEMD(INdata(i,:));
    for j=1:size(imf,1)
        pe1(i,j)=ipe(imf(j,:));
    end
end

xlswrite('E:\毕业课题\我的进展\课题的改进算法研究\EMD和改进EMD的分类结果对比\基于MPE-DCEEMD的特征提取\内圈故障',pe1)
%% 对内圈故障数据进行eemd
...
for i=1:size(OUTdata,1)
    ...
    imf=MPE_DCEEMD(OUTdata(i,:));
    for j=1:size(imf,1)
        pe2(i,j)=ipe(imf(j,:));
    end
end
xlswrite('E:\毕业课题\我的进展\课题的改进算法研究\EMD和改进EMD的分类结果对比\基于MPE-DCEEMD的特征提取\外圈故障',pe2)
%% 对滚珠故障数据进行eemd
...
for i=1:size(BALLdata,1)
    ...
    imf=MPE_DCEEMD(BALLdata(i,:));
    for j=1:size(imf,1)
        pe3(i,j)=ipe(imf(j,:));
    end
end
xlswrite('E:\毕业课题\我的进展\课题的改进算法研究\EMD和改进EMD的分类结果对比\基于MPE-DCEEMD的特征提取\滚珠故障',pe3)
toc
